...
public class CookieManager {
    ...
    */
    ...
    public static boolean VisitedCookie(HttpServletRequest request, HttpServletResponse response, String num) {
        Cookie[] cookies = request.getCookies();
        if (cookies != null) {
            for (Cookie cookie : cookies) {
                if (cookie.getName().equals("Visited-" + num)) {
                    // 이미 방문한 게시물이므로 쿠키를 생성하지 않고 false를 반환한다.
                    return false;
                }
            }
        }

        // 쿠키가 없으므로 쿠키를 생성하고 true를 반환한다.
        Cookie newCookie = new Cookie("Visited-" + num, "true");
        newCookie.setMaxAge(24 * 60 * 60); // 쿠키는 하루 동안만 유효하다.
